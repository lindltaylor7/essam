    ...
    return(
      ...
      <main className="flex flex-col gap-8 row-start-2 items-center sm:items-start">
        <h1 className="text-3xl sm:text-4xl text-center sm:text-left">
          ...
          <input type="text" className="p-2 my-2 bg-zinc-500 rounded" placeholder="Nombre de la cafetería" {...register("name")}/>
          <select className="p-2 my-2 bg-zinc-500 rounded text-white" {...register("unit")}>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {cafes.map(cafe => (
                <tr key={cafe._id}>
                    ...
                    <button className="bg-red-500 text-white px-4 py-2 rounded-md" onClick={deleteCafes(cafe._id)}>Eliminar</button>
                    </td>
                </tr>
            ))}
          </tbody>
